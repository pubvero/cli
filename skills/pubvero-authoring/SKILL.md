...
For result shapes, the security boundary, and currently available preview modes, read [references/runtime.md](references/runtime.md). This repository provides local authoring diagnostics and public discovery checks, not OAuth login or connected local preview. Do not claim those capabilities are available. The hosted draft preview already supports authorized live data; offline fixtures are optional and must remain visibly synthetic.
